<template>
    <v-card flat tile min-height="380" class="d-flex flex-column">
        <confirm ref="confirm" data-app="true"></confirm>
        <v-card-text
            v-if="!path"
        ...
        >File: {{ path }}
        <v-btn  icon title="Create Folder">
            <a target="__blank" :href="endpoints.download.url.replace(new RegExp('{filename}', 'g'), filename).replace(new RegExp('{storage}', 'g'), storage).replace(new RegExp('{path}', 'g'), path)"><v-icon>mdi-file-download</v-icon></a>
        </v-btn>
        </v-card-text>
        <div style="    padding: 20px;
    box-sizing: border-box;" v-if="path.split('.')[path.split('.').length-1] in {'png':'','jpg':''}">
            <img height=200 :src="baseUrl + '/storage/local/download/dsd?path='+path+'&p=_'"  />
        </div>
